scatter = function(data){
  ...
  var yScale = d3.scale.linear()
                  ...
                  .range([0, 500]);
// in d3, 0-0 px is where x/y begins 
// domain input (years)
// range - pixel values, to be mapped from 0 to 500 
  ...
  var bluePoints = wrapper.selectAll('rect')
         ...
         // .attr('height',8)
         .attr('x',function(d,i){
          // specifying where top corner is, so want it to be 5 px left of center
          // circle is automatically at center
          return xScale(new Date(years[i],1,1))-4;
         })
         .attr('y',function(d,i){
          //return the appropriate y value in here
          return yScale(100-d)-4;
         })
         .attr('fill','blue');;
